...
## Particle 类的方法 - createBoom

### 方法介绍

UniCore 内置 Particle 类，提供 createBoom 方法用于创建一个类爆炸粒子效果。
...
let parameter = {
  ...
  image: "../../../static/img/label/boom.png", // 粒子图片资源，必填
...
点击 [在线链接](http://192.168.4.66:8091/?id=createBoom) 以查看在线演示。
  ...
  methods: {
    ...
    */
    init () {
      ...
      // 创建爆炸特效
      let boomParticlePrimitives = uniCore.particle.createBoom(parameter);

      setTimeout(() => {
        this.$message(
          { message: "执行销毁爆炸特效程序" }
        )
        uniCore.particle.deleteParticle(boomParticlePrimitives);
      }, 15000)
